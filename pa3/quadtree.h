...
public:
    ...
    /**
     * Helper function of render to render each node with no non-null children.
     */
    void renderNode(Node* node, PNG &img) const;
    ...
    /**
     * Helper function to prune.
     */
    void pruneNode(Node* node, const int tol);

    /**
     * pruneSizeNode()
     */
    int pruneSizeNode(Node* node, const int tol) const;
     ...
     */
    int idealPruneHelp(int n, int tol, const int leaves) const;

    /**
     * Compares nodes by their variance.
     */
...
private:
    ...
    /**
     * Helper function for clear() that takes a node and deletes its children
     * and itself. 
     */
    ...
    /**
     * Recursive helper function for copy() that takes a node pointer and
     * copies both itself and its children.
     */
    Node* makeCopiedNode(Node* oldNode);
    ...
    virtual int findUpperBound(Node* node) const = 0;
